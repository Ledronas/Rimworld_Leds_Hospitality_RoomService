...
public class ModSettings_RoomService : ModSettings
{
    ...
    public void DoSettingsWindowContents(Rect inRect)
    {
        ...
        // Listing_Standard auto-wraps into a second (horizontally offset) column once cumulative
        // content height crosses the rect height it was given - fine for a genuinely multi-column
        // layout, but inside a vertical BeginScrollView that second column lands outside the
        // clipped area and is invisible, and CurHeight (just curY) then reports only that
        // truncated column's height. Since viewHeight is fed back from CurHeight each frame, one
        // wrap collapses it permanently on every subsequent frame too. maxOneColumn disables the
        // wrap entirely so content just keeps stacking vertically as intended.
